...
* **底层 (Base Layer - 触控板与触觉涟漪)**：
  ...
  * **手势触点**：在 **135°（左上）** 和 **-45°（右下，即 315°）** 的位置绘制两个发光的**淡皮肤色（`#FFCAD4`）**小圆形触点，表示手指接触面。
  * **触觉涟漪**：从两个触点向外辐射 2 圈半径渐变的极细、半透明同心圆波纹，代表触觉马达（Haptics）的震动反馈。
* **中层 (Middle Layer - 物理悬浮阴影)**：
  * 悬浮旋钮的圆环和指针在底层投射一层双重弥散阴影，阴影 Y 轴向下偏移 15px-25px，模拟悬浮高度。
* **顶层 (Top Layer - 2D 全息旋钮)**：
  * **粗圆环**：旋钮的外围圆环线宽**加粗一倍**（从直径的 2% 增至 4%）。
  * **中心指针**：旋钮**去除中心的圆点（空心镂空）**。指针从圆周 135° 位置往内收缩延伸，**指向左上方 45°**。
