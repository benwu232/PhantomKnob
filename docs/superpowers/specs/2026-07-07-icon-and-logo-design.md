...
### 三层图层结构与规范
* **底层 (Base Layer - 触控板与触觉涟漪)**：
  * **触控板底盘**：一个大圆角的方形底盘，具有磨砂玻璃（`#1F222B`）质感和高对比度边框高光，模拟 MacBook 的触控板。
  * **手势触点**：在 **45°（右上）** 和 **-135°（左下，即 225°）** 的位置绘制两个发光的**淡皮肤色（`#FFCAD4`）**大圆形触点（尺寸加大一倍，占大小的 7%），表示手指接触面。
  * **触觉涟漪**：从两个触点向外辐射 2 圈半径扩大一倍的半透明同心圆波纹，代表触觉马达（Haptics）的震动反馈。
* **中层 (Middle Layer - 物理悬浮阴影)**：
  * 悬浮旋钮的圆环和指针在底层投射一层双重弥散阴影，阴影 Y 轴向下偏移 15px-25px，模拟悬浮高度。
* **顶层 (Top Layer - 2D 全息旋钮)**：
  * **粗圆环**：旋钮的外围圆环线宽**再加粗一倍**（从直径的 4% 增至 8%），具有极高饱满度。
  * **中心指针**：旋钮**去除中心的圆点（空心镂空）**。指针线条与圆周外环**完全脱离（相隔 25% 间距）**，从 75% 半径处向中心 25% 半径处延伸，**指向左上方 45° (135° 位置)**，线宽为 6%。
