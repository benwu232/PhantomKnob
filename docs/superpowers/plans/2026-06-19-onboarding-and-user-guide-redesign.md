...
3. 重塑新手引导：
   ...
   - **Step 2**：并排展示“双旋钮”与“无级变速旋钮”的自绘视图，实时显示数值。添加键盘方向键/数字键微调倍率的视觉反馈。支持悬停按下 `C` 键弹出现有的“旋钮定制面板”。
   - **Step 3**：全局热键与完成设置页。
...
- [ ] **步骤 2：重写 UserGuideView 的 Step 2 视图**
  ...
  2. 横向并排渲染两个自绘旋钮，旋钮顶部大字号实时显示当前的数字：
     ...
     - **右侧：无级变速旋钮**（视觉上以渐变射线表示）。悬停时 `step2HoveredType = .linear`。下方附带说明：“线性变速：步长随半径大小无级变速。”
