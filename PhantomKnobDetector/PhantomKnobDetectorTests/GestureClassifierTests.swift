...
final class GestureClassifierTests: XCTestCase {
    ...
    func testCentroidTranslationLocksToPan() {
        ...
        // 模拟在 0.2 秒时发生大位移（向右平移 0.10，位移 0.10 > 0.08 阈值）
        let points2: [Int: CGPoint] = [1: CGPoint(x: 0.4, y: 0.5), 2: CGPoint(x: 0.8, y: 0.5)]
        ...
        let radians = 15 * Double.pi / 180
        let points3: [Int: CGPoint] = [
            1: CGPoint(x: 0.5 - 0.2*cos(radians) + 0.1, y: 0.5 - 0.2*sin(radians)),
            2: CGPoint(x: 0.5 + 0.2*cos(radians) + 0.1, y: 0.5 + 0.2*sin(radians))
        ]
